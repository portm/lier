...
export = (it) => {
    function test (description: string, value, type, expect?) {
        it('validate: ' + description, () => {
            const val = lier.validate(value, type)
            return it.eq(
                val,
                expect,
            ).catch((err) => {
    ...
    test('symbol', Symbol.split, Symbol.split)
    test('symbol err', Symbol.split, Symbol.iterator, [{ path: [], message: [ Symbol.split, 'is not', Symbol.iterator ] }])
